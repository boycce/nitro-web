...
# Export (CSV/XLSX) endpoints
  ...
  Unless explicitly specified, an export reuses the index handler's filters but ignores its sorting/paging — every filtered 
  row goes out in a fixed order. Branch on a query flag (e.g. `?export=1`) before the count/paginate call and return early:

  ```js
  async function find(req, res) {
    try {
      const isExport = !!req.query.export
      const filters = parseFilters(req.query, { ... })
      const query = { company: req.user.company._id, ...filters }
      if (isExport) {
        const rows = await db.invoice.find({ query, sort: { number: 1 }, populate: db.invoice.populate() })
        res.json({ filename: await exportFilename(req, filters), data: exportInvoiceSheet(rows) })
        return
      }
      const total = await db.invoice.count({ query })
      const rows = await db.invoice.find({ query, ...parseSortOptions(req.query, db.invoice, perPage) })
      res.json({ total, rows })
    } catch (err) {
      res.error(err)
    }
  ...
  If the sheet-building logic is only called from this one place, keep it in the same `.api.js` control file rather than
  splitting it into its own file. If it is called in three or more places then split it out to its own file.

  Prefix every export-only function with something like `export*`, and group them at the bottom of the file under a section banner
  (see the structuring skill for the exact banner format):

  ```js
  // ---- export ----------------------------------

  async function exportFilename(req, filters) { ... }
  function exportInvoiceSheet(rows) { ... }     // builds the workbook, calls exportInvoiceRow per row
  function exportInvoiceRow(row) { ... }
  ```
